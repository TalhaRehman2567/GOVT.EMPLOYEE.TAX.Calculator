...
const TAX_SLABS = [
  ...
  { min: 2200000, max: 3200000, base: 116000, rate: 0.20, over: 2200000 },
...
export default function TaxCalculator() {
  ...
  const calculate = () => {
    ...
    const net_annual = annual - tax;
    const net_monthly = net_annual / 12;
    setResult({ annual, tax, monthly_tax, effective, net_annual, net_monthly });
  };
  ...
  const slabRows = TAX_SLABS.map((s, i) => {
    ...
    const isActive =
      result &&
      result.annual > s.min &&
      (s.max === Infinity ? true : result.annual <= s.max);
    return { label: labels[i], rate: rateLabels[i], isActive };
  });
    ...
    <div style={{
      ...
      <div style={{ textAlign: "center", marginBottom: 28 }}>
        <div style={{
          ...
          <span style={{ fontSize: 13, color: "#7eb8f7", letterSpacing: 1.2, textTransform: "uppercase", fontWeight: 600 }}>
        ...
        <p style={{ color: "#94a3b8", fontSize: 14, margin: 0 }}>
          Salaried Individuals · Pakistan Income Tax
        </p>
      </div>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          <button
            ...
            style={{
              ...
              fontWeight: 700, fontSize: 15, cursor: monthly ? "pointer" : "not-allowed",
              transition: "all 0.2s",
              whiteSpace: "nowrap",
            }}
          >
        ...
        {monthly && (
          <p style={{ margin: "10px 0 0", fontSize: 13, color: "#64748b" }}>
            Annual income: {formatPKR(parseFloat(monthly) * 12)}
          </p>
        )}
      </div>
      ...
      {result && (
        <div style={{ marginBottom: 20, animation: "fadeIn 0.4s ease" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
            {[
              { label: "Monthly Tax", value: formatPKR(result.monthly_tax), color: "#f87171", bg: "rgba(239,68,68,0.1)", border: "rgba(239,68,68,0.2)" },
              { label: "Annual Tax", value: formatPKR(result.tax), color: "#fbbf24", bg: "rgba(251,191,36,0.1)", border: "rgba(251,191,36,0.2)" },
              { label: "Net Monthly", value: formatPKR(result.net_monthly), color: "#34d399", bg: "rgba(52,211,153,0.1)", border: "rgba(52,211,153,0.2)" },
              { label: "Net Annual", value: formatPKR(result.net_annual), color: "#60a5fa", bg: "rgba(96,165,250,0.1)", border: "rgba(96,165,250,0.2)" },
      ...
      <div style={{
        ...
        <div style={{
          background: "rgba(37,99,235,0.3)", padding: "14px 18px",
        ...
        {slabRows.map(({ label, rate, isActive }, i) => (
          <div key={i} style={{
            padding: "12px 18px",
            borderBottom: i < slabRows.length - 1 ? "1px solid rgba(255,255,255,0.05)" : "none",
      ...
      <p style={{ textAlign: "center", fontSize: 11, color: "#334155", marginTop: 16 }}>
        Source: Friends Consulting · Federal Budget 2026–27
      </p>
    </div>
  );
}
